...
test_that("define_psa gives correct output", {
  psa <- define_psa(surv_dist_1 ~ use_surv_dist(dist = rexp(1000, 0.5)))
  expect_equal(psa$list_qdist$surv_dist_1, quote(use_surv_dist(dist = rexp(1000, 0.5))), ignore_attr = TRUE)
...
test_that("psa surv_dist is correct", {
  ...
  expect_true(
    abs(resTM$run_model$.cost - resPSA$run_model$cost) < 100
  )
  ...
  psa2 <- define_psa(surv_dist_2 ~
                       resample_surv(1000))
  resPSA <- run_psa(resTM2, psa2, 10)
